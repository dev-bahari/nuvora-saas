...
export class DianDirectSubmissionService {
  ...
  async submit(input: { document: DraftDocument; tenant: TenantInfo; pfx: Buffer; password: string; caChain: Buffer; testSetId: string; source?: { number: string; uuid: string; issueDate: string } }) {
    const credentials = new PfxChainLoaderService().load(input.pfx, input.password, input.caChain);
    const unsigned = new DianUblRendererService().render(input.document, input.tenant, input.source);
    ...
    const response = await this.soap.sendTestSetAsync({ zipBase64: zip.toString('base64'), testSetId: input.testSetId, credentials });
    return { response, signedXml: signed, zip, filename: name };
  }

  poll(input: { trackId: string; credentials: DianSigningCredentials }) { return this.soap.getStatusZip(input); }
}
